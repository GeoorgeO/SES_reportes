...
--set @FechaInicio = convert(nvarchar(4), datepart(year,getdate()) , 0)+''+right('0'+convert(nvarchar(2), datepart(month,getdate()) , 0),2)+''+right('0'+convert(nvarchar(2), datepart(day,getdate()) , 0),2)+' 09:00:00' 
--set @FechaFin = convert(nvarchar(4), datepart(year,getdate()) , 0)+''+right('0'+convert(nvarchar(2), datepart(month,getdate()) , 0),2)+''+right('0'+convert(nvarchar(2), datepart(day,getdate()) , 0),2)+' 09:59:59'

set @FechaInicio = '20180101 00:00:00' 
set @FechaFin = '20191231 23:59:59'



IF EXISTS(
Select * from SES_Reportes.dbo.RPT_VentaAcumuladaSemanal
where FechaInsert between @FechaInicio and @FechaFin AND Sucursal in ('CENTRO','MORELOS','FCOVILLA')
) 
 Begin
...
SET NOCOUNT ON
SET @tableHTML =
    N'<H1>Ventas Acumuladas Semanal</H1>' +
